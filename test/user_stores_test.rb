...
describe 'user stories' do
  ...
  describe 'wave 1' do
    ...
    it 'I can access the list of reservations for a specific date, so that I can track reservations by date' do
      ...
      new_reservation1 = hotel.book_reservation(new_booking_dates)
      new_reservation2 = hotel.book_reservation(new_booking_dates)
    ...
    it 'I want exception raised when an invalid date range is provided, so that I cant make a reservation for an invalid date range' do
      ...
      expect{hotel.book_reservation('booking_date_range')}.must_raise ArgumentError
      expect{hotel.book_reservation('Jan 1, 2019')}.must_raise ArgumentError
  ...
  describe 'wave 2' do
    it 'I can view a list of rooms that are not reserved for a given date range, so that I can see all available rooms for that day' do
      hotel.add_rooms(3,200)
      new_reservation = hotel.book_reservation(new_booking_dates)
      available_rooms = hotel.find_available_rooms_bydate(input_date)
